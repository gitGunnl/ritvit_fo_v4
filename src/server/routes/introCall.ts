...
export const handleIntroCallSubmission = async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      formData.append("entry.1234567890", notes || ''); // Notes field - replace with actual entry ID from Google Forms
      formData.append("entry.0987654321", consent ? 'Yes' : 'No'); // Consent field - replace with actual entry ID from Google Forms
      formData.append("entry.1111111111", JSON.stringify(utm_params)); // UTM tracking - replace with actual entry ID from Google Forms
